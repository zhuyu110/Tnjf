<template>
	<gui-page @scroll="scroll">
	 ...
	 <view slot="gBody" class="gui-flex " >
		 <image src="../static/images/cj_bg.png" mode="widthFix" style="width:750rpx; z-index:-5"></image>
		<image src="../static/images/ic_cjsm.png" class="gui-margin-small"  style="position: absolute;width:750rpx;height: 350rpx; z-index:0;margin-top: 1220rpx;"></image>
		<!-- <image src="../static/images/ic_hbfm.png" class="gui-margin-small" mode="widthFix" style="position: absolute;width:710rpx; z-index:0;margin-top: 1160rpx;"></image>
		<text class="gui-color-white  gui-bold gui-h6 "  style="background: none;position: absolute;z-index: 0;margin-top: 1170rpx;margin-left: 450rpx;">⟦点击领取⟧</text> -->
		<text class="gui-color-black  gui-bold gui-h6 "  style="background: none;position: absolute;z-index: 0;margin-top: 1480rpx;margin-left: 250rpx;">版本号:{{version}}</text>
		<text class="gui-color-yellow  gui-bold gui-h6 "  style="background: none;position: absolute;z-index: 1;margin-top: 950rpx;margin-left: 280rpx;">已有{{totalCount}}人参与</text>
		<image  src="../static/images/ic_rich_bg.png"   mode="widthFix" style="position: absolute;width:650rpx; z-index:0;margin-top: 1050rpx;margin-left: 50rpx;"></image>
		<image src="../static/images/ic_bb_bg.png"   mode="widthFix" style="position: absolute;width:750rpx; z-index:-1;margin-top: 60rpx; opacity:0.7;"></image>
		<image @tap="getzjjl()" src="../static/images/ic_zjjl.png"   mode="widthFix" style="position: absolute;width:150rpx; z-index:99;margin-top: 370rpx;margin-left:550rpx;"></image>
		<text class="gui-color-white  gui-bold gui-h6 "  style="background: none;position: absolute;z-index: 0;margin-top: 1080rpx;margin-left: 260rpx;">{{richHistory}}</text>
		<text class="gui-color-white  gui-bold gui-h6 "  style="background: none;position: absolute;z-index: 0;margin-top: 1080rpx;margin-left: 550rpx;">{{currentRich}}</text>
		
		<view class="gui-margin-top" style="position: absolute;width:750rpx; z-index:0;margin-top: 90rpx;">
			<!-- <gui-scrollitems-y 
			ref="guiscrollitemsy" 
			v-if="winList != null" 
			:items="winList"  duration="2000"></gui-scrollitems-y> -->
			<tonyscroll :autoplay=true
			:datalist="winList"
			:duration="2000"
			></tonyscroll>
		</view>
		<image src="../static/images/ic_dj.png"  mode="widthFix"  style="position: absolute; width:100rpx; margin-top: 20rpx;margin-left: 40rpx;"></image>
		<view class="gui-margin"style="position: absolute;width:750rpx; z-index:0;margin-top: 20rpx;margin-left:150rpx;">
		<gui-speaker
		:items="speakerMsgs" 
		:vertical="true" 
		:current="0"
		:interval="3000"
		styles=" overflow:hidden; height:60rpx; line-height:60rpx; font-size:28rpx; color:#ecec00;">
		</gui-speaker>
		</view>
		
		
		<view  style=" margin: 80rpx; position: absolute;margin-top: 350rpx;" >
		<view class="hb">
			<view class="hb-bd">
			...
			<button   v-show="showCJ" style="border-radius: 50%;background:none   ;z-index: 10;" class="gui-lucky-turntable-btn"    @tap="goDraw">
				<text class="gui-color-white  gui-bold gui-text-small " >\n剩余\n{{cjcount}}\n次抽奖</text>
			</button>
		...
		<!-- <view class="lotteryModal" @touchmove.stop="handle" @click="closeLotteryModal" v-if="lotteryModalShow">
			<view class="lotteryModal-content animate__animated animate__zoomIn" @click.stop="handle">
				<view class="lotteryModal-content-body">
					<image src="../static/images/ic_hb_bg.png" mode="" class="lotteryModal-content-body-bg"></image>
					<view class="lotteryModal-content-body-bd">
						...
						<button v-else class="gui-button lotteryModal-content-body-bd-bg3 " @tap="submitMony(drawId,money)">
							<text class="gui-color-black  gui-bold gui-h2"  >领</text>
						</button>
				...
				<gui-popup ref="guipopup1">
					<view class="gui-flex  gui-img-in ">
						
						<image  src="../static/images/ic_wlhg.png"  mode="widthFix" style="width: 750rpx;height: 650rpx;"
						></image>
						<text class="gui-color-white gui-h6 "  @tap="close1"    style="position: absolute;margin-top: 360rpx;margin-left: 220rpx;">点击领取</text>
						<!-- 关闭按钮 -->
				...
				<gui-popup ref="guipopup4" >
					<view class="gui-flex gui-box-shadow gui-img-in ">
						<!-- <image @tap="goPhoneBtn()" src="../static/images/ic_xxhg.png"  style="width: 600rpx;height: 650rpx;"  
						></image> -->
						<image  src="../static/images/ic_hb_bg.png"  style="width: 600rpx;height: 650rpx;"
						></image>
						<text class="gui-color-yellow gui-h6 "    style="position: absolute;margin-top: 320rpx;margin-left: 200rpx;">{{money}}元现金红包</text>
						
						<text class="gui-color-white gui-h6 "  @tap="submitMony(drawId,money)"    style="position: absolute;margin-top: 405rpx;margin-left: 220rpx;">点击领取</text>
						<!-- 关闭按钮 -->
						<text class="gui-block-text demo-close gui-icons gui-color-white gui-absolute-rt"
						@tap.stop="close4">&#xe78a;</text>
					</view>
				</gui-popup>
				
				<gui-modal ref="guipopup3" title="录入抽奖码">
...
import tonyscroll from '../tony-scroll-self/tony-scroll-self.vue'
...
export default{
	data() {
		return {
			...
			rewardNames    : [
				...
				{"cover": "http://cdn.letwind.com/zy/free/lottery/redpack.png",
				"name": "188.88元"},
				 {"cover": "../../pagesE/static/images/ic_wlmini.png",
				"name": "五菱MINI"}],
			// 奖品展示区背景颜色
	...
	methods:{
		...
		getzjzjmd(){
			var now=this;
			uni.gRequest.get(
					'jfPoints/getWinningInformation',
					{}, 
					(res)=>{
						console.log('get 异步请求成功');
						console.log(res);
						if(res.status=='SUCCESS'){
							console.log("getzjzjmd",res);
							if(res.status=="SUCCESS"){
								now.winList=res.result.winList;
								now.bigWinList=res.result.bigWinList;
										var newData = [];
											// 遍历数据 转换对象格式
											now.bigWinList.forEach((item)=>{
												item.title = "恭喜"+item.mobile+"  "+item.address+"  "+"获得"+item.prize;
												newData.push(item);
											})
											// 转换后将数据赋值到组件
											now.speakerMsgs = newData;
		...
		getzjjl(){
			graceJS.navigate("../pages/cj_recordList")
		},
		goAddress(){
			this.close1();
			graceJS.navigate("../../pages/address/newAddAddress")
		},
		getcjCount(){
			var now=this;
			uni.gRequest.get(
					...
					(res)=>{
						...
						if(res.status=='SUCCESS'){
							...
							if(res.status=="SUCCESS"){
								...
								now.richHistory=res.result.riches;
								now.cjcount=Math.floor(now.currentRich/5);
								now.totalCount=res.result.count;
							}
		...
		open4  : function () {this.$refs.guipopup4.open();},
		close4 : function () {this.$refs.guipopup4.close();},
		open() {
		...
		goDraw(){
			...
			console.log("cjcount=",this.cjcount)
			if(this.cjcount<=0){
				this.showToast("没有抽奖次数，获取财富值得更多抽奖机会吧！")
				return ;
			}
						...
						(res)=>{
							...
							if(res.status=="SUCCESS"){
								graceJS.arrayEach(that.model, (item, index)=>{
												if(item.name==res.result.redpacketAmount){
													that.money=item.name;
													that.start(item.key);
													that.drawId=res.result.id;
												}
			...
			var that=this;
		        uni.getUserProfile({
		            ...
		            success:res=>{
						...
						that.submitMony(that.drawId,that.money);
		            },
		            fail:err=>{
	...
	/* 体现接口 */
		...
		setTimeout(()=>{
			...
			console.log("wxUserInfostr=",JSON.stringify(wxUserInfostr))
					uni.gRequest.post(
					 			'wealth/redpacket/cashout',
					 			{
									data:{
										id:drawId,information:drawContent, wxUserInfo:JSON.stringify(wxUserInfostr)
									}
								},
								'json',
					 			{},
	...
	var drawId=option.drawId;
	var reqtype=option.reqtype;
	var redpacketAmount=option.redpacketAmount;
	if(drawId!=null&&reqtype!=null&&redpacketAmount!=null){
		this.drawId=drawId;
		this.money=redpacketAmount;
		if(reqtype==0){
			this.open4()
		}else{
			this.open1()
	...
	onShow() {
		...
		this.nickName=uni.getStorageSync('wxaccount')
...
.gui-lucky-turntable-btn{width:160rpx; height:160rpx; position:absolute;line-height:30rpx;   left:205rpx; top:190rpx; z-index:9;}
